        ...
        <main className="chat-window" style={{ height: '100%' }}>
          ...
          <div className="messages-area">
            {messages.length <= 0 ? (
              <div className="hero-wrapper">
                <HeroSection handleSendMessage={handleSendMessage} inputText={inputText} setInputText={setInputText} toggleListening={toggleListening} isListening={isListening} isLoading={isLoading} />
              </div>
            ) : (
              <>
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className={`message-row ${msg.sender}`}>
                    ...
                    <div className="message-content-wrapper" style={msg.sender === 'bot' ? { maxWidth: '100%' } : {}}>
                      {msg.image && <img src={msg.image} alt="uploaded" className="chat-image-bubble" />}
                      
                      {/* ✅ เรียกใช้ฟังก์ชัน render ตรงนี้ */}
                      {renderMessageContent(msg)} 
                      
                    </div>
                  </div>
                ))}
                {isLoading && <div className="message-row bot"><div className="bot-icon-chat"><Bot size={20}/></div><div className="message-bubble typing-indicator">...</div></div>}
          ...
          <div className="chat-footer">
              <div className="shortcut-container">
                 {user?.role === 'admin' && <button className="shortcut-chip admin-chip" onClick={() => setInputText('/addmovie ')}>+ เพิ่มหนัง</button>}
              ...
              <div className="input-container">
                ...
                <input type="text" placeholder="พิมพ์ข้อความ..." value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !isLoading && handleSendMessage()} disabled={isLoading} className={isListening ? "listening-mode" : ""} />
